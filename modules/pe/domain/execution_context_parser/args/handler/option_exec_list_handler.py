...
from modules.pe.domain.execution_chain_runner.runner.handler.parametrization.parametrization_context import ParametrizationContext, ParametrizationContextItemSelectionAll, \
    ParametrizationContextItemSelection, ParametrizationContextItemStatementEach
...
from modules.pe.domain.utils import ColorUtils, ArgUtils
...
class OptionExecListHandler(ArgsHandlerAbstract):
    ...
    def print_exec_args_content(self, exec_content: str, prefix: str, out) -> set:
        keys_found = set()
        if exec_content:
            parametrizationContext = ParametrizationContext(exec_content, dict(), SelectionReader(), StatementReader(), False)
            runnerHandlerHelper = RunnerHandlerHelper()
            runnerHandlerHelper.read_input(parametrizationContext)

            keys_found.update(self.print_all_expression(       exec_content, parametrizationContext.items, prefix, out))
            keys_found.update(self.print_selection_expressions(exec_content, parametrizationContext.items, prefix, out))
            keys_found.update(self.print_statement_expressions(exec_content, parametrizationContext.items, prefix, out))
        return keys_found
