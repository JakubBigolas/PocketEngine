...
from modules.pe.domain.execution_chain_runner.runner.handler.parametrization.parametrization_context import ParametrizationContext, ParametrizationContextItemSelectionAll, \
    ParametrizationContextItemSelection
...
class SelectionReader:
    ...
    def read_selection(self, context: ParametrizationContext) -> bool:
    ...
    def read_selection_done(self, context: ParametrizationContext):
        if not context.read_begin("#>>"):
            raise PeError("Selection not closed properly")



    def evaluate_key(self, context: ParametrizationContext, item: ParametrizationContextItemSelection):
    ...
    def validate_key(self, context: ParametrizationContext, item: ParametrizationContextItemSelection):
    ...
    def read_key(self, context: ParametrizationContext):
        key = ""
    ...
    def read_selection_key_array_index(self, context: ParametrizationContext):
        index = None
        if context.read_begin("["):
    ...
    def read_sub_selection(self, context: ParametrizationContext, begin: str, end: list, item: ParametrizationContextItemSelection):
    ...
    def sub_read(self, context: ParametrizationContext, item: ParametrizationContextItemSelection) -> str:
        sub_context = ParametrizationContext(context.input, context.args, context.selection_reader, context.statement_reader, context.throw_on_key_validation)
        try:
